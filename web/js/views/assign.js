...
export function renderAssign(state, params, ctx) {
  ...
  const refreshSaveBtn = () => {
    ...
    saveBtn.textContent = dirty.size
  ...
  for (const d of devices) {
    ...
    // The two dropdowns stay independent. An area with no slot is a real
    // configuration -- not every unit is tied to a serving position -- so it is
    // saved as given rather than argued with.
    locSel.addEventListener('change', () => {
      markDirty(d.device_id, 'location', locSel.value || null, d.location);
      flag();
    });
    slotSel.addEventListener('change', () => {
      markDirty(d.device_id, 'food_slot', slotSel.value ? Number(slotSel.value) : null, origSlot);
      flag();
    });
